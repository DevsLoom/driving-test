...
import QuestionCategoryForm from "~/src/components/panel/pages/Questions/Categories/Form";
...
import { QuestionCategoryType } from "~/src/types/questions/categories";
...
const CategoryList = () => {
    const [items, setItems] = useState<QuestionCategoryType[]>([]);
    ...
    const {
        ...
        isUninitialized,
    } = useFetchQuestionCategoryQuery(selectedId, {
        skip: !selectedId,
    ...
    useEffect(() => {
        if (data && data.length) {
            const mergeData = (
                prevItems: QuestionCategoryType[],
                newItems: QuestionCategoryType[],
    ...
    const [deleteItem, result] = useDeleteQuestionCategoryMutation();
    ...
    return (
        <>
            <AppDialog
                title={`${!isUninitialized && selectedId ? "Update" : "Add"} Category`}
                opened={opened}
                close={closeHandler}
            >
...
CategoryList.layout = (page: any) => (
    <Panel children={page} title="Question Categories" />
);
