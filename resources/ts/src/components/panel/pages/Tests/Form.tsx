...
const TestForm: FC<{
    ...
    const [create, resCr] = useCreateTestMutation();
    const [update, resUp] = useUpdateTestMutation();
    ...
    const errorCallbackHandler = (err: any) => {
        if (err.status === "validate_error") {
            ...
            Object.keys(errors).forEach((fieldName) =>
                setError(fieldName as keyof TestFormType, {
                    type: "manual",
    ...
    const onSubmit = async (form: TestFormType) => {
        if (payload) {
            await update(form)
                .unwrap()
    ...
    useEffect(() => {
        if (payload && Object.keys(payload).length > 0) {
            ...
            Object.keys(payload).forEach((key) => {
                if (payload[key] !== null) {
                    setValue(key as keyof TestFormType, payload[key] ?? "");
                }
            });
        }
    }, [payload]);
