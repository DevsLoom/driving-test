...
class FavoriteController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            $fields    = [primaryKey(), 'user_id', 'model', 'model_id'];
            $condition = [];
            $relations = [];
            $counts    = [];
            $queries   = Favorite::query();
    ...
    public function store(FavoriteRequest $request)
    {
        try {
            ...
            if ($query = Favorite::query()->create($payload)) {
                return entityResponse($query, 201, 'success', 'Favorite added successfully.');
            }
        } catch (Exception $e) {
            return messageResponse($e->getMessage(), 500, 'server_error');
        }
    ...
    public function show(string $id)
    {
        try {
            ...
            if (!$query = Favorite::query()->select($fields)->with($relations)->withCount($counts)->where($condition)->first()) {
                return messageResponse('Favorite not found...', 404, 'error');
            }
    ...
    public function update(FavoriteRequest $request, string $id)
    {
        try {
            ...
            if (!$query = Favorite::query()->where($condition)->first()) {
                return messageResponse('Favorite not found...', 404, 'error');
            }

            $query->update($request->validated());
            return entityResponse($query, 201, 'success', 'Favorite updated successfully.');
        } catch (Exception $e) {
            return messageResponse($e->getMessage(), 500, 'server_error');
        }
    }
    ...
    public function destroy(string $id)
    {
        try {
            ...
            $query->delete();
            return messageResponse('Favorite deleted successfully');
        } catch (Exception $e) {
            return messageResponse($e->getMessage(), 500, 'server_error');
        }
    }
